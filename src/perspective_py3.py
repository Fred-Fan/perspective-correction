...
def filter_perpendicular(lines, margin):
  ...
  for a, b in idxs:
    #print(a,b)
    if abs(abs(lines[a][0][1] - lines[b][0][1]) - perpendicular) < np.pi/6:
      count[a] += 1
      count[b] += 1
...
def eliminate_duplicates(img, lines, threshold):
  ...
  min_distance = max(img.shape) * threshold_distance
  min_theta = np.pi * threshold_distance
  idxs = combinations(range(len(lines)), 2)
  ...
  for i, j in idxs:
    ...
    line_a, line_b = lines[i][0], lines[j][0]
...
def filtercorners(intersections)
    # just get 4 corners that are from 2*2 lines
    index = 0
    dict_corner_index = {}
    dict_corner_origin = {}
    for j in range(len(intersections)):
        for i in range(j, len(intersections[j])):
            if len(intersections[j][i]) > 0:
                dict_corner_index[index] = intersections[j][i]
                dict_corner_origin[index] = [j, i]
                index+=1
    cnts = []
    idxs = combinations(range(index), 4)
    for a, b, c, d in idxs:
    #print(a,b,c,d)
        origin_list = dict_corner_origin[a]+dict_corner_origin[b]+dict_corner_origin[c]+dict_corner_origin[d]
        if (len(set(origin_list))==4) and (Counter(origin_list).most_common(1)[0][1]==2):
        
            #temp = Counter()
            corners =[dict_corner_index[a],dict_corner_index[b],dict_corner_index[c],dict_corner_index[d]]
            row_index = list(zip(*corners))[1]
            if min(row_index)>=200 and max(row_index)<=400:
                cnts.append(np.array(corners, dtype=np.float32))      
    return cnts
